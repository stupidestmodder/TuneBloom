#include "snd/CurveAdshr.h"

#include <basis/seadAssert.h>

namespace snd { namespace internal {

//  square scale - decibel conversion table

const s16 CurveAdshr::cDecibelSquareTable[cDecibelSquareTableSize] = {
...
void CurveAdshr::initialize(f32 initDecibel)
{
    this->setAttack(cAttackInit);
    this->setHold(cHoldInit);
    this->setDecay(cDecayInit);
    this->setSustain(cSustainInit);
    this->setRelease(cReleaseInit);

    this->reset(initDecibel);
}

void CurveAdshr::reset(f32 initDecibel)
{
    mValue = initDecibel * 10.0f;
    mStatus = Status::Attack;
}

void CurveAdshr::update(s32 msec)
{
    switch (mStatus)
    {
        case Status::Attack:
        ...
        case Status::Hold:
        {
            ...
            {
                mHoldCounter -= static_cast<u16>(msec);
            }
            else
            {
                msec -= mHoldCounter;
                mHoldCounter = 0;
                mStatus = Status::Decay;
            }

            if (mStatus != Status::Decay)
                break;
        ...
        case Status::Decay:
        {
            const f32 sustainDecay = CurveAdshr::calcDecibelSquare(mSustain);

            mValue -= mDecay * msec;
            if (mValue < sustainDecay)
            {
                mValue = sustainDecay;
                mStatus = Status::Sustain;
            }
...
f32  CurveAdshr::getValue() const
{
    // Returns the maximum volume without starting volume from zero when attack velocity is at maximum
    if (mStatus == Status::Attack && mAttack == 0.0f)
        return 0.0f;

    return mValue / 10.0f;
}

void CurveAdshr::setAttack(s32 attack)
{
    /*----------------------------------------------------------
    optimize the following calculations in a table

    static const u8 attackTable[127 - 109 + 1] = {
        0, 1, 5, 14, 26, 38, 51, 63, 73, 84,
        92, 100, 109, 116, 123, 127, 132, 137, 143
    };

    SEAD_ASSERT(attack >= 0 && attack <= 127);
...
void CurveAdshr::setHold(s32 hold)
{
    SEAD_ASSERT(hold >= 0 && hold <= 127);

    mHold = static_cast<u16>((hold + 1) * (hold + 1) / 4);
}

void CurveAdshr::setDecay(s32 decay)
{
    SEAD_ASSERT(decay >= 0 && decay <= 127);

    mDecay = CurveAdshr::calcRelease(decay);
}

void CurveAdshr::setSustain(s32 sustain)
{
    SEAD_ASSERT(sustain >= 0 && sustain <= 127);

    mSustain = static_cast<u8>(sustain);
}

void CurveAdshr::setRelease(s32 release)
{
    SEAD_ASSERT(release >= 0 && release <= 127);

    mRelease = CurveAdshr::calcRelease(release);
}

f32 CurveAdshr::calcRelease(s32 release)
{
    SEAD_ASSERT(release >= 0 && release <= 127);
...
s16 CurveAdshr::calcDecibelSquare(s32 scale)
{
    SEAD_ASSERT(scale >= 0 && scale <= cCalcDecibelScaleMax);

    return cDecibelSquareTable[scale];
}
