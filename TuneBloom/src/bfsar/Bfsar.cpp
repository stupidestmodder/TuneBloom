...
*/
    ...
    for (u32 i = 0; i < soundArchive.GetSoundCount(); i++)
    {
        ...
        else if (sound->mSoundType == Sound::SoundType::Wave)
        {
            ...
            if (wsdFile)
            {
                // Item* prevItem = PopupMgr::instance()->getCurrentProcessItem_();
                // PopupMgr::instance()->setCurrentProcessItem(nullptr);
                nw::snd::internal::WaveSoundFileReader reader(wsdFile);
                // PopupMgr::instance()->setCurrentProcessItem(prevItem);
...
bool Bfsar::validate_()
{
    ...
    auto validateList = [&error, this](const ItemList& list, bool validateWaveSoundSet = false, bool validateStreams = false)
    {
        ...
        for (auto it = list.robustBegin(); it != list.robustEnd(); ++it)
        {
            ...
            {
                sound = static_cast<const Sound*>(item);
            }
    ...
    if (!validateList(mSoundList, false, true))
    {
        return false;
    }
